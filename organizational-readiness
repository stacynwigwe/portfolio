---
layout: default
title: "Organizational Readiness Framework"
description: "A structured communication and change enablement model designed to prepare users for new system rollouts. Built from my experience guiding student readiness for a major SIS update at WGU."
permalink: /portfolio/organizational-readiness
tags: [portfolio, systems, change-enable­ment, organizational-readiness, framework, WGU, education, user-adoption]
---

# Organizational Readiness Framework: Preparing People for System Change

## Project Overview
This case study demonstrates how structured communication and change enablement drive successful system adoption across large, distributed environments.  
Drawing from my work at Western Governors University (WGU), I developed an **Organizational Readiness Framework** to prepare students—our end users—for a major update in the **Student Information System (Banner)** that introduced a self-service “What-If Analysis” and Field Experience Checklist.

The initiative aimed to improve **transparency, engagement, and ownership** for students completing field experience requirements, while aligning stakeholders across academic advising, IT, and program leadership.

## The Challenge
Students often reached the final stage of their programs underprepared for fieldwork due to limited visibility into prerequisites such as certification exams, background checks, and fingerprinting.  
This lack of clarity caused delays in term completion and frustration among both students and mentors.

The introduction of the **Field Experience Readiness Checklist** within the SIS was designed to solve this—but success depended on more than technology. It required proactive communication, user education, and guided adoption.

---

## The Framework

### 1. Discovery and Alignment
* Identified recurring student barriers to completing field experience requirements on time.  
* Partnered with leadership to clarify the checklist’s purpose and timeline for rollout.  
* Defined success metrics around adoption rate, student readiness, and mentor feedback.

### 2. Current State Mapping
* Documented the existing student journey from program start to field experience entry.  
* Highlighted communication gaps between students, mentors, and the field experience department.  
* Mapped points of confusion that caused delays in exam scheduling and background clearance.

### 3. Future State Design
* Designed a readiness model that embedded proactive communication earlier in the program lifecycle.  
* Helped outline mentor talking points and visuals for walking students through the new checklist.  
* Collaborated with cross-functional teams to ensure consistent messaging across emails, calls, and student portals.

### 4. Configuration and Testing
* Participated in internal preview sessions of the new system interface.  
* Tested common student scenarios to ensure visibility of checklist items reflected real-time progress data.  
* Gathered feedback from mentors and student pilot users to identify navigation or terminology issues.

### 5. Change Enablement and Training
* Developed communication scripts, email templates, and walkthrough guides for student mentoring sessions.  
* Hosted one-on-one and group mentoring calls to explain the system change, its benefits, and actions required.  
* Reinforced key deadlines and ensured students completed prerequisite steps (e.g., fingerprinting, testing) to activate checklist visibility.

### 6. Continuous Improvement
* Monitored post-launch engagement and student reactions through mentoring notes and follow-up sessions.  
* Shared anecdotal evidence of improved readiness and reduced confusion with leadership.  
* Recommended additional data visibility for mentors to further enhance completion tracking.

---

## Outcome
The initiative increased **student confidence and self-management**, reducing delays caused by missed prerequisites and fostering more proactive planning behavior.  
Mentors reported smoother term transitions, while students expressed greater ownership over their academic and field experience milestones.

By using a structured readiness framework rooted in empathy and communication, this project turned a system update into a meaningful shift in user behavior—strengthening trust between students, mentors, and institutional leadership.

---

## Visual Concepts

### Visual 1: Organizational Readiness Lifecycle

![Organizational Readiness Lifecycle](portfolio/IMG_1708.jpeg)


*A six-stage loop diagram illustrating readiness as an ongoing process*

---

### Visual 2: Empowering Users Through System Transparency
*A two-column map showing how communication translates into confidence and adoption*

Left side (Communication Strategies)  
* Early awareness campaigns  
* Guided training sessions  
* Feedback loops  
* Mentorship support  

Right side (Behavioral Outcomes)  
* Transparency and trust  
* Increased readiness  
* Proactive planning  
* Sustained adoption  

![Empowering Users Through System Transparency](assets/communication-to-adoption-map.png)

---

## Where This Lives
This project belongs in the Main Data Projects Hub under **Enterprise Systems and Change Enablement Strategy.**  
It pairs directly with the **System Implementation Framework**, showing how human-centered readiness ensures successful system adoption.

---

## Résumé Echo
*Led organizational readiness and change enablement efforts for a new SIS feature rollout, preparing students for self-directed field experience tracking through structured communication, training, and adoption support. Applied a six-step framework to improve transparency, confidence, and system utilization across the learner population.*
